% drude.m - Drude model for Silver, Gold, Copper, Aluminum
% 
% Usage: ep = drude(lambda,metal)
% 
% lambda  = vector of wavelengths in nm (may not be infinite)
...
%
% \eps(\om) = 1 + \sum_{i=1}^6 \frac{f_i\om_i^2}{\om_i^2 - \om^2 + j*\ga_i \om} 
%
% use c = 299792.458 nm THz for converting wavelength in nm to f in THz
% also note, 241.7989348 THz = 1 ev
...
c  = 299792.458;     % speed of light in units of nm THz
ev = 241.7989348;    % THz to ev conversion, 241.7989348 THz = 1 ev, or, 1 THz = 1/241.7989348 ev

%      Ag       Au      Cu      Al      % data from Rakic, all frequencies are in units of ev
%     ---------------------------------------------------------------------------------------
A = [ 9.010   9.030   10.83   14.98     % wp    plasma frequency

      0.845   0.760   0.575   0.523     % f0    free-electron Drude oscillator strength
      0.048   0.053   0.030   0.047     % ga0   free-electron Drude damping constant
         0       0       0       0      % w0    free-electron Drude i.e., zero resonant frequency 
...
w = c./lambda/ev;      % convert lambda to frequencies in ev
