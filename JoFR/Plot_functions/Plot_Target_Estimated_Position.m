...
%     title('Position estimation with Particle filter & POMDP.', 'FontSize', 20);
    ...
    for i=1:ntarget
       labelpoints(est.X{i}(1,est.foundIndex{i}(end)) +1,est.X{i}(2,est.foundIndex{i}(end))+1, num2str(i),'FontSize', 10); 
    %    text(est.X{i}(1,est.foundIndex{i}(end)) +1,est.X{i}(2,est.foundIndex{i}(end))+1, num2str(i));
    end
    % axis([0,350,0,350]);
