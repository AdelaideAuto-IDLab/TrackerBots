function Pr = friis_with_fitted_meas(x,uav,yagi_2elements_gain,varargin)
    ...
    % Parse inputs
    parse(p, x,uav,yagi_2elements_gain, varargin{:});
    ...
    pos_relative = ([x;ones(size(x,2),1)]-uav(1:3,:));
    ...
    if ~p.Results.Use2Ray
        ...
        Pr = -39.67	-20*log10(r./Total_Gain) + 10*log10(gh(floor(theta/(2*pi)*N)+1)) + 10*log10(ge(floor(phi/(2*pi)*N)+1)) ;
    end
