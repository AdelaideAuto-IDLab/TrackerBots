function [uavkp1, Reward] = Control_Vector_Selection_Horizon_Uniform_Rect (xk, pf, obs, UAV_Sets, Ms, alpha, model)
    Ns = pf.Ns;
...
%     UAV_Sets = UAV_Sets(:,gain>=prctile(gain,50)); % Get new control action set with good gain only. 80 is the best
    ...
    for a = 1:nu
        u = UAV_Sets(:,a);
        if inpolygon(u(1,k),u(2,k), model.rect.R_x,model.rect.R_y)
            R(a) = -1e30;
        else
            ykp1 = zeros(Ms,1);
