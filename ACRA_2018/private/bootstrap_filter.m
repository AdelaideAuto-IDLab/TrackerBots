...
%         wkm1 = ones(Ns,1)/Ns;
    ...
    if yk >= pf.RSS_Threshold
        ...
        if isfield(pf,'DEM') &&  pf.DEM.UseDEM
            idx = floor(xk([1 2],:));
            idx(idx<1) = 1;
            idx(idx>min(size(pf.DEM.AltTable))) = min(size(pf.DEM.AltTable));
            xk(3,:) = Extract_Alt_From_Table(pf.DEM.AltTable,idx); 
        end
